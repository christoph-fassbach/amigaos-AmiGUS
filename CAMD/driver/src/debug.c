...
// so... rather not do it.
VOID __ASM__ RawPutCharC( __REG__( d0, BYTE putCh )) {
...
VOID debug_fprintf( STRPTR format, ... ) {
  ...
  if ( !AmiGUS_CAMD_Base->agb_LogFile ) {
    ...
    if ( 36 <= (( struct Library *) DOSBase )->lib_Version ) {
      ...
      LONG i = GetVar( "AmiGUS-CAMD-LOG-FILEPATH",
                       buffer,
                       sizeof( buffer ),
                       0 );
      if (( i > 0 ) && ( i < 512 )) {
    ...
    AmiGUS_CAMD_Base->agb_LogFile = Open( logFilePath, MODE_NEWFILE );
  ...
  RawDoFmt(
    ...
    &debug_mPutChProc,
    &printBuffer );
  Write( AmiGUS_CAMD_Base->agb_LogFile, buffer, printBuffer - buffer - 1 );
}
...
VOID debug_mprintf( STRPTR format, ... ) {
  ...
  if ( !AmiGUS_CAMD_Base->agb_LogMem ) {
    ...
    if ( 36 <= (( struct Library * ) DOSBase )->lib_Version) {
       ...
       * 3/4k: setenv AmiGUS-CAMD-LOG-ADDRESS 167772160  -> 0x0a000000
       * 2k:   setenv AmiGUS-CAMD-LOG-ADDRESS 4194304    -> 0x00400000
       */
      ...
      i = GetVar( "AmiGUS-CAMD-LOG-SIZE", buffer, sizeof( buffer ), 0 );
      if ( i > 0 ) {
    ...
    if ( !AmiGUS_CAMD_Base->agb_LogMem ) {
      ...
      AmiGUS_CAMD_Base->agb_LogMem = AllocAbs( size, ( APTR ) 0x0a000000 );
    }
    if ( !AmiGUS_CAMD_Base->agb_LogMem ) {

      AmiGUS_CAMD_Base->agb_LogMem = AllocAbs( size, ( APTR ) 0x00400000 );
    }
    if ( !AmiGUS_CAMD_Base->agb_LogMem ) {

      AmiGUS_CAMD_Base->agb_LogMem = AllocAbs( size, ( APTR ) 0x48000000 );
    }
    if ( !AmiGUS_CAMD_Base->agb_LogMem ) {
    ...
    if ( AmiGUS_CAMD_Base->agb_LogMem ) {
      ...
      LONG i;
      for ( i = 0; i < size; i += 4 ) {
        *( ULONG * )(( LONG ) AmiGUS_CAMD_Base->agb_LogMem + i ) = 0;
      }
    } else {
