...
VOID debug_fprintf( STRPTR format, ... ) {
  ...
  STRPTR logFilePath = "ram:AmiGUS-CAMD-tool.log";
  UBYTE buffer[512];
  ...
  if ( !AmiGUS_CAMD_Tool_Base->agt_LogFile ) {
    ...
    if ( 36 <= (( struct Library *) DOSBase )->lib_Version ) {
      ...
      LONG i = GetVar( "AmiGUS-CAMD-TOOL-LOG-FILEPATH",
                       buffer,
                       sizeof( buffer ),
                       0 );
      if (( i > 0 ) && ( i < 512 )) {
    ...
    AmiGUS_CAMD_Tool_Base->agt_LogFile = Open( logFilePath, MODE_NEWFILE );
  ...
  RawDoFmt(
    ...
    &debug_mPutChProc,
    &printBuffer );
  Write( AmiGUS_CAMD_Tool_Base->agt_LogFile, buffer, printBuffer - buffer - 1 );
}
...
VOID debug_mprintf( STRPTR format, ... ) {
  ...
  if ( !AmiGUS_CAMD_Tool_Base->agt_LogMem ) {
    ...
    if ( 36 <= (( struct Library * ) DOSBase )->lib_Version) {
      ...
      UBYTE buffer[ 64 ];
      LONG i = GetVar( "AmiGUS-CAMD-TOOL-LOG-ADDRESS", buffer, sizeof( buffer ), 0 );
      /*
       * UAE:  setenv AmiGUS-CAMD-TOOL-LOG-ADDRESS 1207959552 -> 0x48000000
       * 3/4k: setenv AmiGUS-CAMD-TOOL-LOG-ADDRESS 167772160  -> 0x0a000000
       * 2k:   setenv AmiGUS-CAMD-TOOL-LOG-ADDRESS 4194304    -> 0x00400000
       */
      ...
      i = GetVar( "AmiGUS-CAMD-TOOL-LOG-SIZE", buffer, sizeof( buffer ), 0 );
      if ( i > 0 ) {
    ...
    debug_kprintf( "AmiGUS-CAMD-TOOL-LOG-ADDRESS %lx = %ld (requested)\n"
                   "AmiGUS-CAMD-TOOL-LOG-SIZE %ld\n",
                   ( LONG ) where,
                   ( LONG ) where,
                   size );
    ...
    if ( 0 < ( LONG ) where ) {
      ...
      AmiGUS_CAMD_Tool_Base->agt_LogMem = AllocAbs( size, where );
    }
    if ( !AmiGUS_CAMD_Tool_Base->agt_LogMem ) {

      AmiGUS_CAMD_Tool_Base->agt_LogMem = AllocAbs( size, ( APTR ) 0x0a000000 );
    }
    if ( !AmiGUS_CAMD_Tool_Base->agt_LogMem ) {

      AmiGUS_CAMD_Tool_Base->agt_LogMem = AllocAbs( size, ( APTR ) 0x00400000 );
    }
    if ( !AmiGUS_CAMD_Tool_Base->agt_LogMem ) {

      AmiGUS_CAMD_Tool_Base->agt_LogMem = AllocAbs( size, ( APTR ) 0x48000000 );
    }
    if ( !AmiGUS_CAMD_Tool_Base->agt_LogMem ) {
    ...
    if ( AmiGUS_CAMD_Tool_Base->agt_LogMem ) {
      ...
      LONG i;
      for ( i = 0; i < size; i += 4 ) {
        *( ULONG * )(( LONG ) AmiGUS_CAMD_Tool_Base->agt_LogMem + i ) = 0;
      }
    } else {

      AmiGUS_CAMD_Tool_Base->agt_LogMem = AllocMem( size, MEMF_CLEAR | MEMF_PUBLIC );
    }
    if ( !AmiGUS_CAMD_Tool_Base->agt_LogMem ) {
    ...
    debug_kprintf( "AmiGUS Log @ 0x%08lx = %ld (retrieved), size %ld\n",
                   ( LONG ) AmiGUS_CAMD_Tool_Base->agt_LogMem,
                   ( LONG ) AmiGUS_CAMD_Tool_Base->agt_LogMem,
                   size );
    ...
    RawDoFmt( "%s %s %s\n",
              ( APTR ) memMarker,
              &debug_mPutChProc,
              &AmiGUS_CAMD_Tool_Base->agt_LogMem );
    /* Move mem blob pointer back to overwrite trailing zero next comment */
    AmiGUS_CAMD_Tool_Base->agt_LogMem =
      ( APTR )(( ULONG ) AmiGUS_CAMD_Tool_Base->agt_LogMem - 1 );
