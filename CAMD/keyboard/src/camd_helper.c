      ...
      FOR_LIST( &cluster->mcl_Senders, next, struct MidiLink * ) {
        ...
        if (( comment ) && ( location )) {
          ...
          if ( labels ) {
            ...
            struct Node * node =
              AllocChooserNode( CNA_Text, name, TAG_END );
            AddTail( labels, node );
          }
        }
      }
...
VOID FreeCamdOutputDevices( struct List * devices, struct List * labels ) {
  ...
  LOG_D(( "V: Labels is empty = %ld\n", IS_EMPTY_LIST( labels )));

  while ( node = RemHead( devices )) {

    struct CAMD_Device_Node * device = node;
    FreeVec( device->cdn_Name );
    FreeMem( device, sizeof( struct CAMD_Device_Node ));
  }
