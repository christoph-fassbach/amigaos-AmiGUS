...
BOOL CheckStartAddress( LONG address ) {
  ...
  result = !strncmp(( UBYTE * ) address, 
                     MEM_LOG_BORDERS, 
                     sizeof( MEM_LOG_BORDERS ) - 1 );
  if ( !result ) return result;

  address += sizeof( MEM_LOG_BORDERS ) - 1;
  result = !strncmp(( UBYTE * ) address, 
                     " " STR( APP_FILE ) " ", 
                     sizeof( " " STR( APP_FILE ) " " ) - 1 );
  if ( !result ) return result;

  address += sizeof( " " STR( APP_FILE ) " " ) - 1;
  result = !strncmp(( UBYTE * ) address, 
                     MEM_LOG_BORDERS, 
                     sizeof( MEM_LOG_BORDERS ) - 1 );
  return result;
...
int main( int argc, char const *argv[] ) {
  ...
  STRPTR filename = "ram:CamdKeyboardMemLog.txt";
  LONG startAddress = 0;
  BOOL found = FALSE;
  ULONG i;
  ...
  */
  for ( i = 1; i < argc; ++i ) {
    ...
    printf( "USAGE:\nGetMemLog [address=0x23443500] [out=t:keyboard-memlog.txt]\n" );
    return 0;
  }
  ...
  if ( !found ) {
    if ( 36 <= (( struct Library * ) DOSBase )->lib_Version) {
      ...
      i = GetVar( "CAMD-KEYBOARD-LOG-ADDRESS", buffer, sizeof( buffer ), 0 );  
      if ( i > 0 ) {
