 ...
 *
 * @param[in]      cardBase    AmiGUS card's base address
...
ASM( LONG ) PlaybackCopy16to8(
  ...
  WriteReg32( cardBase, AMIGUS_PCM_PLAY_FIFO_WRITE, out );
...
ASM( LONG ) PlaybackCopy32to24(
  ...
  WriteReg32( cardBase, AMIGUS_PCM_PLAY_FIFO_WRITE, outA );
  WriteReg32( cardBase, AMIGUS_PCM_PLAY_FIFO_WRITE, outB );
  WriteReg32( cardBase, AMIGUS_PCM_PLAY_FIFO_WRITE, outC );
...
ASM( LONG ) RecordingCopy8Mto16S(
  ...
  ULONG in = ReadReg32( cardBase, AMIGUS_PCM_REC_FIFO_READ );
...
ASM( LONG ) RecordingCopy24Mto32S(
  ...
  ULONG inA = ReadReg32( cardBase, AMIGUS_PCM_REC_FIFO_READ );
  UWORD inB = ReadReg16( cardBase, AMIGUS_PCM_REC_FIFO_READ );
...
ASM( LONG ) RecordingCopy24Sto32S(
  ...
  ULONG inA = ReadReg32( cardBase, AMIGUS_PCM_REC_FIFO_READ );
  ULONG inB = ReadReg32( cardBase, AMIGUS_PCM_REC_FIFO_READ );
  ULONG inC = ReadReg32( cardBase, AMIGUS_PCM_REC_FIFO_READ );
  ULONG outA = (( inA & 0xffFF0000 )       ) | (( ++i ) << 8 );
  ULONG outB = (( inA & 0x0000ffFF ) << 16 ) | (( ++i ) << 8 );
